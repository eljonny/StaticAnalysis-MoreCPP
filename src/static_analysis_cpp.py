...
def append_sarif_issue(is_note, per_issue_string, new_line, list_of_issues):
    if not is_note:
        if len(per_issue_string) > 0 and (per_issue_string not in list_of_issues):
            list_of_issues.append(per_issue_string)
        per_issue_string = new_line
...
def create_comment_for_sarif(
    sarif_output, files_changed_in_pr, output_to_console
):
    """
    Generates a comment for a GitHub pull request based on SARIF output.

    Parameters:
        sarif_output (sarif_json_om): The unparsed output to parse.
        files_changed_in_pr (dict): A dictionary containing the files that were
            changed in the pull request and the lines that were modified.
        output_to_console (bool): Whether or not to output the results to the console.
    ...
    for result in sarif_output.results:
        ...
        file_path = result.locations[0].physicalLocation.artifactLocation.uri.split(":")[1]
        level = result.level
        violation_line = result.locations[0].physicalLocation.region.startLine,
        violation_char = result.locations[0].physicalLocation.region.startColumn,
        issue_detail = result.message.text
        location_ctx = result.codeFlows[0].threadFlows[0].locations[0].location.message.text
        """
        ...
        if utils.is_part_of_pr_changes(file_path, result, files_changed_in_pr):
            ...
            was_note = result.level == "note"
            new_line = utils.generate_outp_from_sarif(
                result,
                description
            )

            if utils.check_for_char_limit(new_line):
                per_issue_string = append_sarif_issue(result, per_issue_string, new_line, list_of_issues)
                utils.CURRENT_COMMENT_LENGTH += len(new_line)
...
def read_files_and_parse_results():
    ...
    # Get flawfinder, cppcheck, fbinfer, and clang-tidy files
    parser = utils.create_common_input_vars_parser()
    parser.add_argument(
        "-ff", "--flawfinder", help="Output file name for flawfinder", required=True
    )
    parser.add_argument(
        "-cc", "--cppcheck", help="Output file name for cppcheck", required=True
    )
    parser.add_argument(
        "-fi", "--fbinfer", help="Output file name for FB Infer", required=True
    )
    parser.add_argument(
    ...
    flawfinder_file_name = parser.parse_args().flawfinder
    cppcheck_file_name = parser.parse_args().cppcheck
    fbinfer_file_name = parser.parse_args().fbinfer
    ...
    flawfinder_comment, flawfinder_issues_found = create_comment_for_sarif(
        flawfinder_content, files_changed_in_pr, output_to_console
    )
    cppcheck_comment, cppcheck_issues_found = create_comment_for_sarif(
        cppcheck_content, files_changed_in_pr, output_to_console
    )
    fbinfer_comment, fbinfer_issues_found = create_comment_for_sarif(
        fbinfer_content, files_changed_in_pr, output_to_console
    )
    ...
    if output_to_console and (flawfinder_issues_found or cppcheck_issues_found or fbinfer_issues_found or clang_tidy_issues_found):
        ...
        if fbinfer_issues_found:
            print(f"{error_color}fbinfer results: {fbinfer_comment}")
...
def prepare_comment_body(
    ...
    if flawfinder_issues_found == 0 and cppcheck_issues_found == 0 and \
        ...
        if len(flawfinder_comment) > 0:
            full_comment_body += (
                f"<details> <summary> <b> :red_circle: flawfinder found "
                f"{flawfinder_issues_found} {'issues' if flawfinder_issues_found > 1 else 'issue'}!"
                " Click here to see details. </b> </summary> <br>"
                f"{flawfinder_comment} </details>"
            )

        full_comment_body += SEPARATOR
        ...
        if len(fbinfer_comment) > 0:
            full_comment_body += (
                f"<details> <summary> <b> :red_circle: FB Infer found "
                f"{fbinfer_issues_found} {'issues' if fbinfer_issues_found > 1 else 'issue'}!"
                " Click here to see details. </b> </summary> <br>"
                f"{fbinfer_comment} </details>"
            )
...
if __name__ == "__main__":
    (
        flawfinder_comment_in,
        cppcheck_comment_in,
        fbinfer_comment_in,
        clang_tidy_comment_in,
        flawfinder_issues_found_in,
        cppcheck_issues_found_in,
        fbinfer_issues_found_in,
